...
export const getStoredWorkspaceState = (): WorkspaceStorageState => {
    ...
    try {
        ...
        const workspaces = Object.values(parsed.workspaces ?? {}).reduce<Record<string, StoredWorkspaceAccount>>((acc, candidate) => {
        ...
        const requestedActiveId = typeof parsed.activeWorkspaceId === "string" && parsed.activeWorkspaceId.length ? parsed.activeWorkspaceId : null;
        const activeWorkspaceId = requestedActiveId && workspaces[requestedActiveId] ? requestedActiveId : fallbackActiveId;
