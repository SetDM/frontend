...
export function ProspectPanel({
        ...
        const interval = setInterval(() => {
            setCountdowns((prev) => {
                ...
                queuedMessages.forEach((message) => {
                    ...
                    next[message.id] = typeof previous === "number" ? Math.max(0, previous - 1) : Math.max(0, message.sendsIn - 1);
                });
                return next;
            });
        }, 1000);
    ...
    if (!prospect) {
        return (
            <div className={cn("flex h-full w-full flex-col overflow-auto border-t border-border bg-card p-4", "md:w-80 md:border-l md:border-t-0", className)}>
        ...
        <div className={cn("flex h-full w-full flex-col overflow-auto border-t border-border bg-card p-4", "md:w-80 md:border-l md:border-t-0", className)}>
                ...
                <div className="flex flex-col items-center text-center">
                    <div className="relative">
                        <Avatar className="h-16 w-16">
                            ...
                            <AvatarFallback className="bg-gradient-to-br from-primary/20 to-pink/20 text-foreground text-lg font-medium">{initials}</AvatarFallback>
                        </Avatar>
                        <div className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-pink text-[10px] font-bold text-pink-foreground">!</div>
                    </div>
                    ...
                    <div className="mt-3 flex flex-col gap-2 text-xs text-muted-foreground">
                        ...
                        {typeof followsBusiness === "boolean" && (
                            <div>
                                <span className="font-medium text-foreground">{followsBusiness ? "Yes" : "No"}</span> follows your business
                            </div>
                        )}
                        {typeof businessFollowsUser === "boolean" && (
                            <div>
                                <span className="font-medium text-foreground">{businessFollowsUser ? "Yes" : "No"}</span> business follows this user
                            </div>
                        )}
                    </div>
                    ...
                    {prospect.isFlagged && (
                        <div className="mt-4 w-full">
                            <Button variant="destructive" className="w-full" disabled={!onClearFlag || isClearingFlag} onClick={handleClearFlag}>
                    ...
                    <div className="mt-4 flex w-full items-center justify-between rounded-lg bg-secondary/50 px-3 py-2">
                        ...
                        <div className="flex items-center gap-2">
                            <Switch checked={prospect.autopilotEnabled} disabled={autopilotUpdating || prospect.isFlagged} onCheckedChange={(checked) => onToggleAutopilot?.(prospect.id, checked)} />
                            {autopilotUpdating && <span className="text-xs text-muted-foreground">Updating...</span>}
                        </div>
                    </div>
                    {prospect.isFlagged && <p className="mt-2 text-xs text-muted-foreground">Remove the flag to re-enable autopilot.</p>}
                </div>
            </div>
            ...
            <div className="border-b border-border p-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-baseline gap-1">
                        <span className={cn("text-2xl font-bold", prospect.leadScore >= 70 ? "text-green-500" : prospect.leadScore >= 40 ? "text-yellow-500" : "text-red-500")}>
                            {prospect.leadScore}
                        </span>
                ...
                <div className="mt-2 h-2 w-full rounded-full bg-muted overflow-hidden">
                    <div
                        className={cn("h-full rounded-full transition-all", prospect.leadScore >= 70 ? "bg-green-500" : prospect.leadScore >= 40 ? "bg-yellow-500" : "bg-red-500")}
            ...
            <div className="border-b border-border p-4">
                ...
                {prospect && onRefreshAiNotes && (
                    <Button size="sm" variant="outline" className="mb-2 w-full" disabled={aiNotesLoading} onClick={() => onRefreshAiNotes(prospect.id)}>
                        {aiNotesLoading ? "Updating notes..." : "Refresh AI notes"}
                    </Button>
                )}
                <div className="space-y-2 rounded-lg bg-secondary/30 p-3">
                    {aiNotesLoading && <p className="text-xs text-muted-foreground">{aiNotes.length ? "Refreshing AI notes..." : "Generating AI notes..."}</p>}
                    {aiNotes.length === 0 && !aiNotesLoading ? <p className="text-xs text-muted-foreground">No AI notes yet.</p> : null}
                    {aiNotes.length > 0 && (
            ...
            <div className="border-b border-border p-4">
                ...
                {queuedMessages.length === 0 ? (
                    <p className="mt-3 text-xs text-muted-foreground">Autopilot has no pending replies for this conversation.</p>
                ) : (
                    <div className="mt-3 space-y-3">
                        {queuedMessages.map((message) => {
                            const countdown = typeof countdowns[message.id] === "number" ? countdowns[message.id] : message.sendsIn;
                            const isCancelling = queueCancelBusyIds.includes(message.id);
                            const isSendingNow = queueSendBusyIds.includes(message.id);

                            return (
                                <div key={message.id} className="rounded-lg border border-primary/20 bg-primary/5 p-3">
                                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                                        <span className="font-semibold uppercase tracking-wide text-primary">Autopilot queued</span>
                                        <span>Sends in {formatTime(Math.max(0, countdown))}</span>
                                    </div>
                                    <p className="mt-2 text-sm text-foreground">{message.content}</p>
                                    <div className="mt-2 text-[11px] text-muted-foreground">{formatScheduledLabel(message.scheduledFor)}</div>
                                    <div className="mt-3 flex flex-col gap-2">
                                        <Button
                                            size="sm"
                                            className="w-full"
                                            disabled={!prospect || !onSendQueuedMessageNow || isSendingNow || isCancelling}
                                        ...
                                        <Button
                                            ...
                                            variant="outline"
                                            className="w-full"
                                            disabled={!prospect || !onCancelQueuedMessage || isCancelling || isSendingNow}
                ...
                {queuedMessages.length > 0 && <p className="mt-3 text-[11px] text-muted-foreground">Canceling removes the response from the queue.</p>}
            </div>
        </div>
    );
}
