...
const FUNNEL_STAGES: FunnelStage[] = ["responded", "lead", "qualified", "booking-sent", "call-booked", "sale", "flagged"];
...
const extractNotesFromPayload = (payload: unknown): string[] => {
    ...
    return maybeNotes.map((note) => (typeof note === "string" ? note.trim() : "")).filter((note): note is string => Boolean(note));
};
...
const normalizeRealtimeQueueEntries = (entries: ConversationRealtimeQueueEntry[] = []) => {
    return entries
        .map((entry, index) => {
            ...
            const scheduledFor = typeof entry?.scheduledFor === "string" && entry.scheduledFor.length > 0 ? entry.scheduledFor : null;

            return {
                id: typeof entry?.id === "string" && entry.id.length > 0 ? entry.id : `queued-${index}`,
                content,
                scheduledFor,
...
const normalizeRealtimeMessage = (conversationId: string, realtime?: ConversationRealtimeMessagePayload["message"]): Message | null => {
    if (!realtime) {
        return null;
    }
    ...
    const timestampInput =
        realtime.timestamp instanceof Date || typeof realtime.timestamp === "number" ? realtime.timestamp : typeof realtime.timestamp === "string" ? realtime.timestamp : new Date().toISOString();
...
const deriveConversationId = (record: ConversationRecord) => {
    ...
    const fallbackRecipient = typeof record.recipientId === "string" && record.recipientId.length > 0 ? record.recipientId : "business";
    const fallbackSender = typeof record.senderId === "string" && record.senderId.length > 0 ? record.senderId : "prospect";
...
const buildProspectFromRecord = (
    ...
    const fallbackIdentity = derivedSenderId || (typeof record.conversationId === "string" ? record.conversationId : null) || instagramId;
    ...
    const normalizedProfileUsername = profile?.username && profile.username.trim().length > 0 ? profile.username.replace(/^@+/, "").trim() : undefined;
    const normalizedMetadataHandle = metadataHandleRaw && metadataHandleRaw.length > 0 ? metadataHandleRaw.replace(/^@+/, "").trim() : undefined;
    ...
    const sanitizedHandleSource = typeof handleSource === "string" ? handleSource.replace(/^@+/, "").trim() : "";
    const resolvedHandleBase = sanitizedHandleSource.length > 0 ? sanitizedHandleSource : fallbackIdentity;
    const normalizedHandle = resolvedHandleBase.startsWith("@") ? resolvedHandleBase : `@${resolvedHandleBase}`;
    ...
    const resolvedDisplayName = (profile?.name && profile.name.trim()) || resolvedUsername || metadataName || handleDisplayFallback || fallbackIdentity;
    const lastMessage = messages[messages.length - 1];
    const followerCount = typeof profile?.followerCount === "number" && Number.isFinite(profile.followerCount) ? profile.followerCount : null;
    const autopilotEnabledFromRecord = typeof record.isAutopilotOn === "boolean" ? record.isAutopilotOn : false;
...
const buildConversationFromRecord = (record: ConversationRecord, profile?: InstagramUserProfile | null, options?: { messageLimit?: number }): Conversation => {
    const conversationId = deriveConversationId(record);
    const rawMessages = Array.isArray(record.messages) ? record.messages : [];
    const requestedLimit = typeof options?.messageLimit === "number" && Number.isFinite(options.messageLimit) ? options.messageLimit : null;
    ...
    const fallbackTimestamp = rawMessages.length > 0 ? rawMessages[rawMessages.length - 1]?.timestamp : undefined;
    ...
    const queuedMessages = queuedMessagesRaw
        .map((queued, index) => {
            ...
            const sendsInSeconds = scheduledDate ? Math.max(0, Math.floor((scheduledDate.getTime() - now) / 1000)) : 0;

            const normalized: QueuedMessage = {
                id: typeof queued?.id === "string" && queued.id.length > 0 ? queued.id : `${conversationId}-queued-${index}`,
                content,
    ...
    return {
        id: conversationId,
        prospect: buildProspectFromRecord(record, conversationId, orderedMessages, fallbackTimestamp, profile),
...
const extractUserProfile = (payload: unknown): InstagramUserProfile | null => {
    ...
    const followerCount = typeof followerCountValue === "number" && Number.isFinite(followerCountValue) ? followerCountValue : null;
    ...
    return {
        ...
        followerCount,
        isUserFollowBusiness: typeof data.isUserFollowBusiness === "boolean" ? data.isUserFollowBusiness : null,
        isBusinessFollowUser: typeof data.isBusinessFollowUser === "boolean" ? data.isBusinessFollowUser : null,
...
export default function Messages() {
    ...
    const [clearFlagBusyConversationId, setClearFlagBusyConversationId] = useState<string | null>(null);
    ...
    const fetchAiNotesForConversation = useCallback(
        async (conversationId: string, options?: { signal?: AbortSignal }) => {
            ...
            try {
                ...
                if (!abortSignal?.aborted) {
                    setAiNotesRequestConversationId((current) => (current === conversationId ? null : current));
                }
            }
        },
        [authorizedFetch]
    ...
    const fetchProfilesForRecords = useCallback(
        async (records: ConversationRecord[], signal?: AbortSignal) => {
            const senderIds = Array.from(new Set(records.map((record) => getSenderIdFromRecord(record)).filter((id): id is string => typeof id === "string" && id.length > 0)));
    ...
    const loadConversationPage = useCallback(
        async ({ limit, skip, stageTag, signal }: LoadConversationPageArgs) => {
            ...
            const items = records.map((record) => {
                const conversationId = deriveConversationId(record);
                if (conversationId && Array.isArray(record.aiNotes) && record.aiNotes.length > 0) {
                    const normalizedNotes = record.aiNotes.map((note) => (typeof note === "string" ? note.trim() : "")).filter((note): note is string => Boolean(note));
    ...
    const mergeFetchedConversations = useCallback((incoming: Conversation[], mode: "replace" | "append") => {
        setConversations((prev) => {
            const prevMap = new Map(prev.map((conversation) => [conversation.id, conversation]));

            const mergeWithHydratedData = (fresh: Conversation, existing: Conversation): Conversation => ({
                ...fresh,
            ...
            incoming.forEach((conversation) => {
                ...
                next[index] = hydratedConversationIdsRef.current[conversation.id] ? mergeWithHydratedData(conversation, existing) : conversation;
            });
            return next;
        });
    }, []);
    ...
    const hydrateConversationDetail = useCallback(
        async (conversationId: string, options?: { signal?: AbortSignal; force?: boolean; messageLimit?: number }) => {
            ...
            const requestedLimit = Math.min(Math.max(Math.floor(requestedLimitRaw) || INITIAL_CONVERSATION_MESSAGE_LIMIT, 1), MAX_CONVERSATION_MESSAGE_LIMIT);
        ...
        [authorizedFetch, fetchProfilesForRecords, getConversationMessageLimit]
    );
    ...
    const handleRealtimeMessage = useCallback(
        (payload: ConversationRealtimeMessagePayload = {}) => {
            ...
            const relativeTimestamp = formatRelativeTime(payload.lastUpdated ?? new Date().toISOString());
            ...
            setConversations((prev) => {
                ...
                const next = prev.map((conversation) => {
                    ...
                    const hasMessageAlready = conversation.messages.some((message) => message.id === normalizedMessage.id);

                    if (hasMessageAlready && conversation.prospect.lastMessage === normalizedMessage.content && conversation.prospect.lastMessageTime === relativeTimestamp) {
                        return conversation;
                    }
                    ...
                    const updatedMessages = hasMessageAlready ? conversation.messages : [...conversation.messages, normalizedMessage];
        ...
        [ensureConversationMessageLimit, getConversationMessageLimit, hydrateConversationDetail, refreshConversations, selectedConversationId]
    );
        ...
        (payload: ConversationRealtimeUpsertPayload = {}) => {
            ...
            setConversations((prev) => {
                ...
                const next = prev.map((conversation) => {
                    ...
                    const resolvedStage = typeof payload.stageTag === "string" && payload.stageTag.length > 0 ? normalizeStageTag(payload.stageTag) : conversation.prospect.stage;

                    const resolvedAutopilot = typeof payload.isAutopilotOn === "boolean" ? payload.isAutopilotOn : conversation.prospect.autopilotEnabled;

                    const resolvedFlagged = typeof payload.isFlagged === "boolean" ? payload.isFlagged : resolvedStage === "flagged";

                    if (resolvedStage === conversation.prospect.stage && resolvedAutopilot === conversation.prospect.autopilotEnabled && resolvedFlagged === Boolean(conversation.prospect.isFlagged)) {
                        return conversation;
                    }
    ...
    }, [authToken, autoRefreshEnabled, handleRealtimeMessage, handleRealtimeQueueUpdate, handleRealtimeUpsert]);
    ...
    }, [hasMoreConversations, isFetchingMoreConversations, isLoading, loadConversationPage, mergeFetchedConversations, selectedFilter]);
    ...
    }, [ensureConversationMessageLimit, getConversationMessageLimit, hydrateConversationDetail, hydratedConversationIds, selectedConversationId]);
    const prospects = useMemo(() => conversations.map((conversation) => conversation.prospect), [conversations]);
    ...
    const selectedProfile = selectedProspect?.instagramId ? profilesById[selectedProspect.instagramId] ?? null : null;
    const currentAiNotes = selectedConversationId ? aiNotesByConversationId[selectedConversationId] ?? [] : [];
    const aiNotesLoading = Boolean(selectedConversationId && aiNotesRequestConversationId === selectedConversationId);
    const isSelectedConversationLoadingOlderMessages = Boolean(selectedConversation && loadingOlderMessageIds.includes(selectedConversation.id));
    ...
    const handleLoadOlderMessages = useCallback(
        async (conversationId: string) => {
            ...
            const nextLimit = Math.min(currentLimit + CONVERSATION_MESSAGE_PAGE_SIZE, MAX_CONVERSATION_MESSAGE_LIMIT);

            setConversationMessageLimits((prev) => ({ ...prev, [conversationId]: nextLimit }));
            setLoadingOlderMessageIds((prev) => (prev.includes(conversationId) ? prev : [...prev, conversationId]));
        ...
        [getConversationMessageLimit, hydrateConversationDetail]
    );
    ...
    const handleToggleAutopilot = useCallback(
        async (conversationId: string, enabled: boolean) => {
            ...
            try {
                ...
                const responseData = payload && typeof payload === "object" ? (payload as { isAutopilotOn?: unknown }) : null;
                const nextValue = typeof responseData?.isAutopilotOn === "boolean" ? responseData.isAutopilotOn : enabled;
    ...
    const handleCancelQueuedMessage = useCallback(
        async (conversationId: string, queuedMessageId: string) => {
            ...
            setQueueCancelBusyIds((prev) => (prev.includes(queuedMessageId) ? prev : [...prev, queuedMessageId]));

            try {
                const response = await authorizedFetch(CONVERSATION_ENDPOINTS.cancelQueuedMessage(conversationId, queuedMessageId), {
                    method: "DELETE",
                });
                ...
                setConversations((prev) =>
                    prev.map((conversation) => {
                        ...
                        return {
                            ...conversation,
                            queuedMessages: conversation.queuedMessages.filter((message) => message.id !== queuedMessageId),
                        };
                    })
                );
    ...
    const handleSendQueuedMessageNow = useCallback(
        async (conversationId: string, queuedMessageId: string) => {
            ...
            setQueueSendBusyIds((prev) => (prev.includes(queuedMessageId) ? prev : [...prev, queuedMessageId]));

            try {
                const response = await authorizedFetch(CONVERSATION_ENDPOINTS.sendQueuedMessageNow(conversationId, queuedMessageId), {
                    method: "POST",
                });
                ...
                const payloadMessage = payload && typeof payload === "object" && "message" in payload ? (payload as { message?: unknown }).message : null;

                const messageContent =
                    payloadMessage && typeof payloadMessage === "object" && typeof (payloadMessage as { content?: unknown }).content === "string"
                        ? ((payloadMessage as { content: string }).content as string)
                        : "";

                const timestampRaw =
                    payloadMessage && typeof payloadMessage === "object" && typeof (payloadMessage as { timestamp?: unknown }).timestamp === "string"
                        ? ((payloadMessage as { timestamp: string }).timestamp as string)
                        : new Date().toISOString();

                const formattedTimestamp = formatMessageTimestamp(timestampRaw);
                const messageId =
                    payloadMessage && typeof payloadMessage === "object" && typeof (payloadMessage as { id?: unknown }).id === "string" && (payloadMessage as { id: string }).id.length > 0
                ...
                setConversations((prev) =>
                    prev.map((conversation) => {
                        ...
                        const filteredQueue = conversation.queuedMessages.filter((message) => message.id !== queuedMessageId);
    ...
    const handleClearFlag = useCallback(
        async (conversationId: string) => {
            ...
            try {
                ...
                const responseData = payload && typeof payload === "object" ? (payload as { stageTag?: unknown }) : null;
                const stageTag = typeof responseData?.stageTag === "string" ? responseData.stageTag : null;
                    ...
                    prev.map((conversation) => {
                        ...
                        return {
                            ...
                            prospect: {
                                ...conversation.prospect,
                                isFlagged: false,
                                stage: conversation.prospect.stage === "flagged" ? normalizedStage : conversation.prospect.stage,
                            },
                        };
                    })
                );
                ...
                setClearFlagBusyConversationId((current) => (current === conversationId ? null : current));
            }
        },
        [authorizedFetch, refreshConversations, setConversations, setError]
    );
    ...
    const handleSendMessage = useCallback(
        async (conversationId: string, content: string) => {
            ...
            try {
                ...
                const payloadMessageRaw = payload && typeof payload === "object" && "message" in payload ? (payload as { message?: unknown }).message : null;
                ...
                const timestampInput = payloadMessage && typeof payloadMessage.timestamp === "string" ? payloadMessage.timestamp : new Date().toISOString();

                const remoteMessageId = payloadMessage && typeof payloadMessage.id === "string" && payloadMessage.id.length > 0 ? payloadMessage.id : null;
                const remoteMetadataMid =
                    payloadMessage && payloadMessage.metadata && typeof payloadMessage.metadata.mid === "string" && payloadMessage.metadata.mid.length > 0
    ...
    if (!isLoading && conversations.length === 0) {
        ...
        const emptyButtonLabel = selectedFilter !== "all" || searchQuery ? "View all conversations" : "Refresh";
            ...
            <AppLayout>
                <div className="flex h-full max-h-screen flex-col items-center justify-center gap-4 text-center">
                    <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground max-w-md">{error ?? "We'll start populating this inbox as soon as new Instagram DMs arrive."}</p>
                    </div>
                    <Button onClick={handleEmptyStateAction} disabled={isLoading || isRefreshing} variant={hasFilterApplied ? "outline" : "default"}>
                        {emptyButtonLabel}
                    </Button>
                </div>
            </AppLayout>
    ...
    return (
        <AppLayout>
            <div className="flex h-full max-h-screen flex-col overflow-hidden">
                {error && conversations.length > 0 && (
                    <div className="mx-4 mb-2 rounded-md border border-destructive/30 bg-destructive/10 px-4 py-2 text-sm text-destructive">
                        <div className="flex items-center justify-between gap-3">
                            <span>{error}</span>
                            <Button variant="ghost" size="sm" onClick={() => refreshConversations()} disabled={isLoading || isRefreshing}>
                                Retry
                            </Button>
                        </div>
                    </div>
                ...
                {(isLoading || isRefreshing) && conversations.length > 0 && <div className="mx-4 mb-2 text-xs text-muted-foreground">Refreshing conversations…</div>}
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    {!isMobile ? (
                        <ProspectPanel
                            ...
                            autopilotUpdating={Boolean(selectedProspect && autopilotBusyConversationId === selectedProspect.id)}
